...
def fix_refs_safe(schema: dict, schema_dir: Path) -> dict:
    ...
    while node_stack:
        ...
        if isinstance(current_node, dict):
            ...
            if '$ref' in current_node:
                ...
                content, object_name = get_referenced_content(ref_file_path, object_path)
    ...
    for object_name, content in inserted_content.items():
        if object_name in schema['definitions']:
            logger.warning(f"Object '{object_name}' already exists in schema definitions. Overwriting with content from {ref_file_path}.")
...
def get_referenced_content(ref_file_path: Path, object_path: str) -> Tuple[dict, str]:
    ...
    if content is None:
        raise ValueError(f"Referenced object '{object_name}' not found in '{ref_file_path}'")
