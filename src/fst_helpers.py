...
def paradigm_cache(current_file: str, cache_dir=".cache/") -> paradigms.Paradigm:
    """
    Arguments:
        current_file:  The file path of the current module.
        cache_dir:     Directory to store cached Paradigm files.
    Returns:
        p:              Paradigm loaded from cache or generated by the decorated function.
    
    Decorator that caches the output of a Paradigm-generating function.
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            cache_path = os.path.join(
                cache_dir,
                f"{cache_key}.paradigm"
            )
            if cache_is_updated(current_file, cache_path):
                with open(cache_path, 'rb') as f:
                    print(
                        f"Loaded paradigm for function {func.__name__} "+\
                        f"with args {args_str} from cache {cache_path}"
                    )
                    p = pickle.load(f)
            else:
                print(
                    f"Building paradigm for function {func.__name__} "+\
                    f"with args {args_str} and cache {cache_path}"
                )
                p = func(*args, **kwargs)
