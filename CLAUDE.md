...
- Validate config YAML against JSON schemas: `python -m src.config_utils.schema_validation` (runs `validate_files_by_type` for every `CONFIG_KINDS` entry against `config/` by default).
