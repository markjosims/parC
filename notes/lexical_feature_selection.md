...
Otherwise, we need to cyclically insert `<Sigma>*` e.g. "`[class_marker=j]<Sigma>*[fv_class=aᴐ]<Sigma>*[BOW]<Sigma>*`"
...
- [ ] `Paradigm` class prepends `[feature=value]` to each root for each lexically specified feature
    - [x] Function `Paradigm.get_lexical_feature_transducer()` returns FST mapping e.g. vəlɛð -> vəlɛð[aᴐ]
